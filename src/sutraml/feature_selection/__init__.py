...
class Wrapper_Methods:
    ...
    def _validate_parameters(self):
        ...
        valid_model_type = ['cls', 'reg']
        valid_device = ['cpu', 'cuda', 'gpu']
        ...
        if not isinstance(self.device, str):
            raise TypeError(f"device must be a str, got {type(self.device)}")
        ...
        if not isinstance(self.bins, int):
            ...
            #         warnings.warn(f"bins values '{value}' must be a float, got {type(value)}")
    ...
    def _load_data(self) -> Tuple[pd.DataFrame, pd.Series]:
        try:
            ...
            elif self.device is 'gpu':
                import cudf
                # Convert the pandas DataFrame to a cuDF (GPU) DataFrame
    ...
    def _load_preprocessor(self, random_state: int = None) -> Pipeline:
        try:
            ...
            if self.scaler_type == 'standard':
                pipeline_steps.steps.append(('scaler', StandardScaler()))
            else:
                pipeline_steps.steps.append(('scaler', MinMaxScaler()))

            full_pipeline = Pipeline(pipeline_steps.steps)

            return full_pipeline
        except Exception as e:
            raise RuntimeError(f"Error in _load_preprocessor : {e}")

    def gpu_balanced_bagging_classifier(self, estimator:str, n_estimators :int):
        """ 
        this bagging classifier will add 
        classifiers: LogisticRegression, SVC, KNN, NB
        regressor: LinearRegressor, SVR, KNN, NB
        """
        try:
            # minority , majority data

            for _ in self._generate_random_seeds(n_seeds=self.seed):

                # GPU balanced sampling
                X_balanced, y_balanced = self.gpu_balance(self.X, self.y)

                # clone model
                model = clone_gpu_model(self.bas)
            pass
        except Exception as e:
            raise RuntimeError(f"Error in _get_balanced_bagging_classifier: {e}")
